...
// Overload that supports emissive contribution.
// Pass in Emission as (emissionTexture.rgb * emissionColor.rgb) from Shader Graph.
void ToonShading_float(in float3 Normal, in float ToonRampSmoothness, in float3 ClipSpacePos, in float3 WorldPos, in float4 ToonRampTinting,
    in float ToonRampOffset, in float3 Emission, out float3 ToonRampOutput, out float3 Direction)
{
    ...
    // multiply with shadows;
    toonRamp *= light.shadowAttenuation;
    // add in lights, extra tinting and emission
    ToonRampOutput = light.color * (toonRamp + ToonRampTinting) + Emission;
    // output direction for rimlight
    Direction = light.direction;
#endif
